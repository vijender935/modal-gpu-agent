...
RAG_CAPTION_ENDPOINT = os.getenv(
    "RAG_CAPTION_ENDPOINT",
...
async def run_python(
    ...
    """Run Python code in a fully unsandboxed environment (network + secrets available).

    WARNING: This is intentionally unrestricted. The code can access the network,
    environment secrets, and has higher resource limits. Use only with trusted prompts.
    Preinstalled packages only (no dynamic pip install).
    """
    ...
    try:
        response = await _post(
            ...
            timeout=MAX_SANDBOX_TIMEOUT + 60.0,
        )
    except RuntimeError as exc:
        return f"Error: {exc}"
    if response.status_code != 200:
...
async def generate_rag_captions(
    ...
    """Generate production-level multimodal RAG captions using Qwen2.5-VL on H100.

    Downloads images from Drive INPUT folder, runs Qwen2.5-VL captioning on H100,
    and uploads structured JSONL (caption, tags, outfit, intensity, nsfw_level, etc.)
    to the Drive OUTPUT folder.

    Start with max_items=10-20 for testing. Then increase.
    """
    if not isinstance(max_items, int) or isinstance(max_items, bool) or max_items < 1 or max_items > 500:
        return "Error: max_items must be an integer between 1 and 500"
    if not isinstance(start_from, int) or isinstance(start_from, bool) or start_from < 0:
        return "Error: start_from must be a non-negative integer"
    if not isinstance(model_name, str) or not model_name.strip():
        return "Error: model_name must be a non-empty string"
    if not isinstance(output_filename, str) or not output_filename.endswith(".jsonl"):
        return "Error: output_filename must end with .jsonl"
    ...
    try:
        response = await _post(
            ...
            timeout=60.0,  # queues the job; actual work is long-running
        )
    except RuntimeError as exc:
        return f"Error: {exc}"
    ...
    try:
        ...
        return "Error: RAG caption endpoint returned invalid JSON"
