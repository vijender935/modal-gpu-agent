...
def _execute_python_sandbox(code: str, timeout_seconds: int = 300) -> dict:
    """Fully unsandboxed Python execution (network + secrets available).

    WARNING: This runs with full access to the environment. Use only in trusted contexts.
    """
    ...
    # Fully unsandboxed: keep all environment variables (including secrets)
    env = dict(os.environ)
    env.update({"PYTHONUNBUFFERED": "1", "PYTHONDONTWRITEBYTECODE": "1"})

    with tempfile.TemporaryDirectory(prefix="modal-unsandbox-") as tempdir:
        ...
        with stdout_path.open("wb") as stdout, stderr_path.open("wb") as stderr:
            process = subprocess.Popen(
                [sys.executable, "main.py"],  # removed -I to allow site-packages fully
                cwd=workdir,
                env=env,
                stdin=subprocess.DEVNULL,
                stdout=stdout,
                stderr=stderr,
                # preexec_fn removed → no resource limits
            )
            try:
...
@app.function(
    ...
    cpu=4,
    memory=16384,
    timeout=MAX_SANDBOX_TIMEOUT + 60,
    # Fully unsandboxed: network + Modal access enabled
    max_containers=5,
)
def run_python_sandbox_cpu(code: str, timeout_seconds: int = 300):
...
def run_python_sandbox_gpu(code: str, timeout_seconds: int = 300):
    return _execute_python_sandbox(code, timeout_seconds)
...
def run_python_sandbox_endpoint(
    ...
    # Dynamic package installation still disabled for safety (pre-installed packages only)
    requirements = item.get("requirements", [])
    if requirements:
        raise HTTPException(
            status_code=422,
            detail="Dynamic package installation is disabled; use the prebuilt image packages",
        )
